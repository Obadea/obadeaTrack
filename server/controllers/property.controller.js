...
//       return res
//         .status(200)
//         .json({ message: "Property removed from saved successfully" });
//     } else {
//       // Add the property to savedProperties
...
//   try {
//     const { propertyId, email, title, location, price, photo } = req.body;
...
//     if (user.savedProperties.some((property) => property._id === propertyId)) {
//       return user.savedProperties.filter((user) => user._id !== propertyId);
...
const getAllSavedProperties = async (req, res) => {
  ...
  try {
    const count = await SavedProperty.countDocuments({ query });

    const allSavedProperties = await SavedProperty.find(query)
      .limit(_end)
      .skip(_start)
      .sort({ [_sort]: _order });
    ...
    res.status(200).json(allSavedProperties);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
